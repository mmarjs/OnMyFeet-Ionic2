...
export class UserProvider {
  ...
  facebookSyncProfile(fbData: any) {
    ...
    if (!currentUser.get('username') && fbData.username) {
      currentUser.set('username', fbData.username);
    }
    
    if (!currentUser.get('first_name') && fbData.name) {
      let name_arr = fbData.name.split(" ");
      currentUser.set('first_name', name_arr[0]);
      currentUser.set('last_name',  name_arr[1]);
    }

    if (!currentUser.get('first_name') && fbData.first_name) {
      currentUser.set('first_name', fbData.first_name);
    }

    if (!currentUser.get('last_name') && fbData.last_name) {
      currentUser.set('last_name', fbData.last_name);
    }
    ...
    if (!currentUser.get('birthday') && fbData.birthday) {
      currentUser.set('birthday', new Date(fbData.birthday));
    }
